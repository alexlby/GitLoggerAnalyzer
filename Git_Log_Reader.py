...
def convert_git_log_file_2_git_commits(file_name):
    ...
    with codecs.open(file_name, 'rt',  'utf-8') as file:
        ...
        for line in all_lines:
            if "commit" in line and line.find('commit') == 0: # filter the case of 'commit' in raw commit message
                ...
                if len(st_ids_in_this_commit) == 0 and len(st_ids_in_this_commit) == 0 and len(ta_ids_in_this_commit) == 0:
                ...
                while not ("commit" in line_pr and line_pr.find('commit') == 0):
                    ...
                    line_pr = all_lines[start_index_of_change_files_in_this_commit_block + count_of_change_files_in_this_commit_block]
                count_of_change_files_in_this_commit_block -= 1

                for currentPr in range(start_index_of_change_files_in_this_commit_block, start_index_of_change_files_in_this_commit_block + count_of_change_files_in_this_commit_block):
...
#     insert_git_commits_2_mysql('SPS', 'develop', git_commits)
#     print ("reader end!")
