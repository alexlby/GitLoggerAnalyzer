...
def output_2_xls(project_name, branch_name):
    ...
    sheet_file_defect = output_workbook.add_sheet('file - defect' + '(' + str(len(file_defect_rows)) + ')')
    if len(file_defect_rows) > 0:
    ...
    sheet_file_story = output_workbook.add_sheet('file - story' + '(' + str(len(file_story_rows)) + ')')
    if len(file_story_rows) > 0:
    ...
    sheet_file_defect_count = output_workbook.add_sheet('file - defect count' + '(' + str(len(file_defect_count_rows)) + ')')
    if len(file_defect_count_rows) > 0:
    ...
    sheet_file_story_count = output_workbook.add_sheet('file - story count' + '(' + str(len(file_story_count_rows)) + ')')
    if len(file_story_count_rows) > 0:
...
def generate_bar_chart(project_name, branch_name):
    ...
    if len(select_file_defect_count_rows) > 0:
        if len(select_file_defect_count_rows) > 20:
            ...
            charGen.barChart(select_file_defect_count_rows[0:], "dist/file_defect_count.png", "File - Related Defects Count Chart", "Related Defects Count", "File Name", pycha.bar.VerticalBarChart, "blue")
    ...
    if len(select_file_story_count_rows) > 0:
        if len(select_file_story_count_rows) > 20:
            ...
            charGen.barChart(select_file_story_count_rows[0:], "dist/file_story_count.png", "File - Related Stories Count Chart", "Related Stories Count", "File Name", pycha.bar.VerticalBarChart, "green")
